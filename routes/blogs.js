...
router.get("/get-one/:id", async function (req, res) {
  const blogs = await db().collection("sample_blogs").findOne({
    id: req.params.id,
  });
  res.json({
    success: true,
...
router.post("/create-one", async function (req, res, next) {
  const blogs = await db().collection("sample_blogs").insertOne({
    title: req.body.title,
    text: req.body.text,
...
router.delete("/single/:blogTitle", async (req, res) => {
  ...
  await db()
    ...
    .deleteOne(titleToDelete, (err, _result) => {
      if (err) {
        res.status(400).send(`Error deleting blog${titleToDelete.id}!`);
      } else {
...
router.delete("/delete-one/:id", async function (req, res) {
  const blogs = await db().collection("sample_blogs").deleteOne({
    id: req.params.id,
  });
  res.json({
    success: true,
